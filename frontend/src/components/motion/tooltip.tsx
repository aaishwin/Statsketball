...
export function Tooltip({
  ...
  /* Handlers live on the wrapper span rather than cloneElement-injected
     onto the child: no ref access during render, no clobbering the child's
     own handlers, and focus/hover bubble up from the trigger naturally. */
  return (
    <>
      <span
        ref={anchorRef}
              ...
              {open && coords ? (
                ...
                >
                  <motion.span
                    ...
                    className={cn(
                      "block whitespace-nowrap rounded-lg border border-border-strong bg-surface-overlay px-2.5 py-1 text-xs font-medium text-ink shadow-lg",
                      className,
                    )}
                  >
                    {content}
